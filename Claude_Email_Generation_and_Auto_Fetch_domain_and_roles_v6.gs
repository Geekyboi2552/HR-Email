...
//  5. Run enrichment first: 📧 Email Recruiter → Enrich Missing Domain & Roles
//  6. Then send emails:     📧 Email Recruiter → Send Emails to All
// ════════════════════════════════════════════════════════════
...
function enrichDomainAndRoles() {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    // Skip if row is empty or both domain + roles already filled
    if (!company) {
      skippedCount++;
      continue;
    }
    if (domain && roles) {
      Logger.log(`Row ${i + 1} (${company}): already has domain + roles — skipping.`);
      skippedCount++;
      continue;
    }
    ...
    try {
      ...
      // Write Domain back to sheet (only if currently blank)
      if (!domain && result.domain) {
        sheet.getRange(i + 1, COL_DOMAIN + 1).setValue(result.domain);
      }

      // Write Roles back to sheet (only if currently blank)
      if (!roles && result.roles && result.roles.length > 0) {
        sheet.getRange(i + 1, COL_ROLES + 1).setValue(result.roles.join(", "));
      }

      // Highlight enriched cells in light blue so you can review them
      if (!domain) sheet.getRange(i + 1, COL_DOMAIN + 1).setBackground("#cfe2f3");
      if (!roles)  sheet.getRange(i + 1, COL_ROLES  + 1).setBackground("#cfe2f3");

      enrichedCount++;
      Logger.log(`  ✓ Domain: ${result.domain} | Roles: ${result.roles.join(", ")}`);
...
{"domain":"<domain>","roles":["<role1>","<role2>","<role3>"]}`;
