...
function enrichDomainAndRoles() {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    while (attempts < MAX_ATTEMPTS && result === null) {
      attempts++;
      try {
        result = fetchDomainAndRoles(company, cfg.ANTHROPIC_API_KEY);
      } catch (e) {
        if (attempts < MAX_ATTEMPTS) {
          Logger.log(`  ↻ Attempt ${attempts} failed for "${company}" — retrying in 5s… (${e.message})`);
          Utilities.sleep(5000);
        } else {
          Logger.log(`  ✗ All attempts failed for "${company}": ${e.message}`);
          failedCount++;
        }
      }
    }
    ...
    if (result) {
      ...
      enrichedCount++;
      Logger.log(`  ✓ Domain: ${result.domain} | Roles: ${result.roles.join(", ")} | Location: ${result.roleLocation}`);
    }

    // Pause between companies — Sonnet + multi-search needs more breathing room
...
// ── SECTION 8a: Multi-source enrichment (Claude Sonnet + web search) ──
//
//  Uses a multi-turn agentic conversation so Claude searches each source
//  deliberately and exhaustively before giving up:
//    Pass 1 → Company careers page + official website
//    Pass 2 → LinkedIn Jobs (site:linkedin.com/jobs)
//    Pass 3 → Glassdoor (site:glassdoor.co.in)
//    Pass 4 → Instahyre / Naukri / Simplify / Shine as fallback
//
//  Uses claude-sonnet for better reasoning across multiple search results.
...
function fetchDomainAndRoles(companyName, apiKey) {
  ...
  const systemPrompt = `You are a company intelligence researcher specialising in Indian companies. 
Your job is to find accurate, current job openings and company information by searching multiple sources.
You MUST search all of the following sources before concluding — do not stop after one search:
  1. The company's own careers/jobs page (search: "${companyName} careers site" or "${companyName}.com/careers")
  2. LinkedIn Jobs (search: "${companyName} jobs site:linkedin.com/jobs" or "linkedin.com/company/${companyName.toLowerCase().replace(/\s+/g,'-')}/jobs")
  3. Glassdoor (search: "${companyName} jobs site:glassdoor.co.in")
  4. Naukri.com (search: "${companyName} jobs site:naukri.com")
  5. If still not enough data, try Instahyre, Shine, or Simplify.

Synthesise results across all sources. Prefer the most recently posted roles.
If a source returns no results, move on to the next — do NOT give up early.`;

  const userPrompt = `Research the company "${companyName}" thoroughly using multiple job sources and return:

1. PRIMARY DOMAIN — The industry/sector it operates in (2–4 words, e.g. "Fintech Payments", "Quick Commerce", "B2B SaaS", "Edtech", "D2C FMCG", "IT Consulting", "Healthtech")

2. OPEN ROLES — Up to 8 specific job titles currently being hired for. Use exact role names as posted (e.g. "Product Manager – Growth", "Senior Data Analyst", "Area Sales Manager"). Do NOT use generic titles like "Various roles".

3. ROLE LOCATION — Indian cities where these roles are based. Comma-separated if multiple (e.g. "Bengaluru, Mumbai", "Pan India", "Remote", "Gurugram"). Default to "India" only if truly not found anywhere.

Search ALL sources listed in your instructions before responding.
...
{"domain":"<domain>","roles":["<role1>","<role2>","<role3>","<role4>","<role5>"],"roleLocation":"<cities>"}`;
  ...
  // Extract text blocks — web_search adds tool_use/tool_result blocks, ignore those
  let text = "";
  ...
  if (start === -1 || end === -1) {
    throw new Error(`No JSON in response for "${companyName}": ${text.substring(0, 300)}`);
  }
  ...
  const roles        = Array.isArray(parsed.roles)
                         ? parsed.roles.map(r => r.trim()).filter(Boolean)
                         : [];
  const roleLocation = (parsed.roleLocation || "India").trim();

  if (!domain) throw new Error(`Empty domain returned for "${companyName}"`);
  if (roles.length === 0) throw new Error(`No roles found for "${companyName}"`);

  return { domain, roles, roleLocation };
}
...
// ── SECTION 9b: Send emails using Google Doc template (no AI generation) ─
//
//  Instead of calling Claude to write each email, this reads your Google Doc
//  template from Email_Doc_Folder, fills {{placeholders}} using the row data,
//  and sends the same template body personalised per HR contact.
//
//  Placeholders supported in your Doc:
//    {{HR_name}}, {{company}}, {{domain}}, {{roles}}, {{roleLocation}},
//    {{designation}}, {{Location}}, {{YOUR_NAME}}, {{YOUR_PHONE}},
//    {{YOUR_LINKEDIN}}, {{YOUR_COLLEGE}}, {{YOUR_BATCH}}
// ──────────────────────────────────────────────────────────────────────────
function sendRecruitingEmailsFromDoc() {

  // Step 1: Load config
  const cfg = getConfig();

  // Step 2: Fetch CV blob (for attachment) — text extraction not needed here
  const cvBlob = getCVBlob(cfg["CV_Folder_ID"]);

  // Step 3: Read Doc template ONCE before the loop
  const docBodyTemplate = getDocBody(cfg["Email_Doc_Folder_ID"]);
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    // Build values for placeholder replacement
    const values = {
      ...cfg,
      HR_name:        row[COL_NAME]          || "",
      company:        row[COL_COMPANY]        || "",
    ...
    const subject = fillTemplate(cfg["Email_Subject"], values);
    const body    = fillTemplate(docBodyTemplate,      values);
    ...
    try {
      ...
      sheet.getRange(i + 1, COL_STATUS + 1).setValue("Sent");
      sheet.getRange(i + 1, COL_STATUS + 1).setBackground("#b7e1cd");
      sentCount++;
      ...
      sheet.getRange(i + 1, COL_STATUS + 1).setValue("Failed");
      sheet.getRange(i + 1, COL_STATUS + 1).setBackground("#f4c7c3");
      Logger.log(`Failed for ${email}: ${e}`);
      failedCount++;
    }
  }
  ...
  const summary = `Sent: ${sentCount} | Skipped: ${skippedCount} | Failed: ${failedCount}`;
  Logger.log(`✅ Doc template send complete! ${summary}`);
  return summary;
}
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addSeparator()
    .addItem("② Send Emails — AI Generated",           "sendRecruitingEmails")
    .addItem("② Send Emails — Doc Template",           "sendRecruitingEmailsFromDoc")
    .addSeparator()
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  #descAI  { border-left-color: #1a73e8; }
  #descDoc { border-left-color: #1e8e3e; }
...
<div class="section" style="margin-bottom:10px;">
  ...
  <div class="section-body">
    ...
    <!-- Mode toggle -->
    <p class="section-desc" style="margin-bottom:8px;">Choose how to generate your email body:</p>
    <div class="mode-toggle">
      <button class="mode-btn active" id="modeAI" onclick="setMode('ai')">✦ AI Generate</button>
      <button class="mode-btn" id="modeDoc" onclick="setMode('doc')">📄 Doc Template</button>
    </div>

    <!-- Mode descriptions -->
    <p class="mode-desc" id="descAI" style="display:block;">Claude writes a unique, personalised email for each HR using their company data and your CV.</p>
    <p class="mode-desc" id="descDoc" style="display:none;">Your Google Doc template is used as the email body. {{placeholders}} are filled per HR row.</p>
...
<script>
  ...
  function setMode(mode) {
    ...
    document.getElementById('descAI').style.display  = mode === 'ai'  ? 'block' : 'none';
    document.getElementById('descDoc').style.display = mode === 'doc' ? 'block' : 'none';
  }
  ...
  function runSend() {
    ...
    const modeLabel = currentMode === 'doc' ? 'Doc Template' : 'AI Generate';
    ...
    const ticker = setInterval(function() {
      ...
      bar.style.width = Math.min(pct, 85) + '%';
      if (currentMode === 'doc') {
        msg.textContent = pct < 40 ? 'Reading Doc template…' : 'Sending via Gmail…';
      } else {
        msg.textContent = pct < 25 ? 'Extracting CV text…'
                        : pct < 55 ? 'Generating personalised emails…'
                        : 'Sending via Gmail…';
      }
    }, 1200);

    const fn = currentMode === 'doc'
      ? google.script.run.withSuccessHandler(onSendDone).withFailureHandler(onSendFail).sendRecruitingEmailsFromDoc()
      : google.script.run.withSuccessHandler(onSendDone).withFailureHandler(onSendFail).sendRecruitingEmails();

    function onSendDone(result) {
      clearInterval(ticker);
      bar.style.width = '100%';
